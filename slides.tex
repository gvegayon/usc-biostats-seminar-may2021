...
\begin{frame}
	...
	\begin{itemize}
		\item 77 experimentally annotated trees
		\item We only used trees in which 
		$$2^{(\max{\mbox{Polytomy}} + 1)\times \mbox{nfuns}}< 0.5\times 10^9$$
		\noindent i.e., half billion
		\item Both models used "informative" priors.
		\item Both were fitted using Adaptive Metropolis (\textbf{fmcmc} R package).
	\end{itemize}
\end{frame}
...
After initializing a \texttt{geese} object named \texttt{model2fit}:
