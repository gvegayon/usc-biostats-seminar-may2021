...
\frame{\frametitle{Contents}
	\tableofcontents
	\vfill
	You can download the slides from\large{} {\color{usccardinal}\href{https://ggv.cl/slides/uai-ds-may}{ggv.cl/slides/uai-ds-may}}
	\normalsize
}
...
%	\vfill \hfill \hyperlink{geneontology}{\beamerreturnbutton{go back}}
	\Large \vfill\hfill\uncover<5->{... Where is all this data?}\normalsize
\end{frame}
...
\begin{frame}[t, label=aphylo-good]
	...
	\begin{minipage}[m]{.65\linewidth}
		...
		\mode<beamer>{
			\only<1>{\includegraphics[width=.85\linewidth]{example-trees-PTHR11258-GO:0001730-1.pdf}}%
			\only<2->{\includegraphics[width=.85\linewidth]{example-trees-PTHR11258-GO:0001730.pdf}}
		}%
		\mode<handout>{
			\includegraphics[width=.85\linewidth]{example-trees-PTHR11258-GO:0001730.pdf}
		}
...
\begin{frame}[t,label=inno-cat1]
	%\frametitle{Models of Functional Evolution}
	\begin{figure}
	...
	\end{figure}
	\vfill\hfill%
	\beamergotobutton{\hyperlink{inno-cat2}{More}}
\end{frame}
...
\begin{center}
	\begin{minipage}[m]{.49\linewidth}
		    \mode<beamer>{%
			\includegraphics<1>[width=.95\linewidth]{geese-equation-likelihood-1.pdf}%
			\includegraphics<2->[width=.95\linewidth]{geese-equation-likelihood.pdf}
		}%
		\mode<handout>{%
			\includegraphics[width=.95\linewidth]{geese-equation-likelihood.pdf}
		}
	\end{minipage}%
\hfill %
	\begin{minipage}[m]{.49\linewidth}
	\mode<beamer>{%
		\includegraphics<3->[width=.95\linewidth]{geese-equation-prediction.pdf}
	}%
	\mode<handout>{%
		\includegraphics[width=.95\linewidth]{geese-equation-prediction.pdf}
	}
\end{minipage}

\end{center}
\vfill\hfill\uncover<4->{... I implemented this (and more) on \textbf{geese}}
\end{frame}
...
\uncover<3->{\byside{.33}{Today}{
    ...
    \begin{itemize}
        ...
        \item \textbf{geese} (Sufficient Stats.): a general framework for gene function evolution.
    \end{itemize}
}}\hfill
...
\includegraphics[height=2cm]{aphylo-data-2.png}
    \begin{itemize}
        ...
        \item<5-> Fit pooled data models (flocks).
...
\begin{frame}[c]
	...
	\textbf{%
		\color{uscgold}
		\Large Triadas, lazos y funciones genéticas\\cuando las redes sociales y la filogenética se encuentran\large %
	}
...
\includemedia[%
	...
	\begin{center}
	\scalebox{2}{\textcolor{uscgold}{Gracias!}} 
	\end{center}
\end{frame}
...
\begin{frame}[t,label=inno-cat2]
	...
	\end{figure}
	\vfill\hfill%
	\beamerreturnbutton{\hyperlink{inno-cat1}{return}}
\end{frame}
